function out = Blockhemis2new(X1,Y1,Z1,R1,X2,Y2,Z2,R2,X3,Y3,Z3,R3,Surf_X)
%
% Blockhemis2.m
%
...
model.param.set('x_1', strcat(num2str(X1(1),'[m]')));
model.param.set('y_1', strcat(num2str(Y1(1),'[m]')));
model.param.set('z_1', strcat(num2str(Z1(1),'[m]')));
model.param.set('r_1', strcat(num2str(R1(1),'[m]')));
model.param.set('R_1', '0.2[m]', 'Radius of the big sphere');
model.param.set('x_cp', '0[m]');
model.param.set('y_cp', '0[m]');
model.param.set('z_cp', '0[m]');
model.param.set('x_2', strcat(num2str(X2(1),'[m]')));
model.param.set('y_2', strcat(num2str(Y2(1),'[m]')));
model.param.set('z_2', strcat(num2str(Z2(1),'[m]')));
model.param.set('r_2', strcat(num2str(R2(1),'[m]')));
model.param.set('x_3', strcat(num2str(X3(1),'[m]')));
model.param.set('y_3', strcat(num2str(Y3(1),'[m]')));
model.param.set('z_3', strcat(num2str(Z3(1),'[m]')));
model.param.set('r_3', strcat(num2str(R3(1),'[m]')));
...
model.component('comp1').geom('geom1').feature('dif1').selection('input').set({'sph1' 'sph2' 'sph3' 'sph4'});
...
model.component('comp1').physics('ht').feature('bt1').feature('bh2').selection.set([2 3 4]);
...
model.component('comp1').physics('ht').feature('hf1').selection.set([2 3 12 13 18 19 20 21 26 27 28 29]);
...
model.result('pg1').feature('surf1').set('rangecolormax', 312);
...
model.result.export('data2').set('data', 'ps1');
model.result.export('data3').set('data', 'ps1');
for i  = 1:length(X)     
            model.param.set('x_1', strcat(num2str(X(i)),'[m]'));%
            model.param.set('y_1', strcat(num2str(Y(i)),'[m]'));%
            model.param.set('z_1', strcat(num2str(Z(i)),'[m]'));%
            model.param.set('r_1', strcat(num2str(R1(i)),'[m]'));%
            model.param.set('R_1', strcat(num2str(R2),'[m]'), 'Radius of the big sphere');
            model.mesh('mesh1').run;
            model.geom('geom1').run;
            model.sol('sol1').runAll;
              
              fprintf('X : %d, Y : %d, Z : %d, R : %d, \n',X(i),Y(i),Z(i),R1(i));
              for iter = 1:length(Surf_X)
               model.result.dataset('cpt1').set('pointx',Surf_X(iter,1));
               model.result.dataset('cpt1').set('pointy',Surf_X(iter,2));
               model.result.dataset('cpt1').set('pointz',Surf_X(iter,3));
               T(i,iter) = model.result.numerical('pev2').getReal();          
              end
end
out = model;
